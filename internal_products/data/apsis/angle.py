...
def haversine(grid: tuple[np.ndarray, np.ndarray], target: tuple[float, float]) -> np.ndarray:
    """Compute the angular distance between an array of latitude/longitude
    points and a target point.

    Parameters
    ----------
    grid
        The [latitude, longitude] of the grid. The latitude and longitude must
        have the same shape and are assumed to be in degrees.
    target
        The [latitude, longitude] of the target point. Both values are assumed
        to be in degrees.
    ...
    >>> haversine((np.array([0]), np.array([0])), (0, 180))
    array([180.])

    Get the angle between any point on the equator and the pole.

    >>> haversine((np.array([0]), np.array([34])), (90, 300))
    array([90.])
