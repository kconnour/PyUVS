...
from internal_products.data.orbit.spacecraft_geometry import \
    make_spacecraft_position_iau_mars_frame, \
    make_spacecraft_velocity_iau_mars_frame
...
def add_spacecraft_position_to_file(group: Group, hduls: list[hdulist]) -> None:
    dataset = group.create_dataset(
        'spacecraft_position',
        data=make_spacecraft_position_iau_mars_frame(hduls),
...
def add_spacecraft_velocity_to_file(group: Group, hduls: list[hdulist]) -> None:
    dataset = group.create_dataset(
        'spacecraft_velocity',
        data=make_spacecraft_velocity_iau_mars_frame(hduls),
        compression=compression,
        compression_opts=compression_opts)
