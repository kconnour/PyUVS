...
def plot_apoapsis_topographic_globe_upright(orbit: int) -> None:
    ...
    It would be straightforward to generalize this function to work for
    creating a globe from any vantage point, but I guess I've never had a need
    to do that.
    ...
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
...
def plot_apoapse_muv_dayside_globe_upright(orbit: int) -> None:
    ...
    projected from the point of apoapsis for a given orbit.
    ...
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-dayside_fromjay.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
    
def plot_apoapse_muv_dayside_globe_upright_reversed(orbit: int) -> None:
    """Plot an upright globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit.
    ...
    #  all at once, otherwise there would be rogue integrations between the
    #  swath boundaries.
    for swath in np.unique(swath_number)[::-1]:
        swath_indices = swath_number == swath
    ...
    # Save the graphic
    #save_location = iuvs_images_location / 'apoapse' / 'muv' / 'globe-heq-upright-dayside-reversed'
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-dayside-reversed.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
...
def plot_apoapse_muv_dayside_globe_rotated(orbit: int) -> None:
    ...
    projected from the point of apoapsis for a given orbit. The rotation makes
    the globe appear in the same orientation that IUVS saw Mars.
    ...
    transform = pu.graphics.compute_rotated_transform(
        mars_position, mars_velocity, subspacecraft_latitude,
        subspacecraft_longitude, globe)
    axis = plt.axes(bbox, projection=projection)
...
def plot_apoapse_muv_daynight_globe_upright_no_nightglow(orbit: int) -> None:
    """Plot an upright globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. The upright
    orientation is such that north is up. The nightside
    data shown is the NO nightglow.
    ...
    # Save the graphic
    #save_location = iuvs_images_location / 'apoapse' / 'muv' / 'globe-heq-upright-nightside'
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-nightside-no_nightglow.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
    
def plot_apoapse_muv_daynight_globe_upright_no_nightglow_reversed(orbit: int) -> None:
    """Plot an upright globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. Swaths are plotted in 
    reverse order to mitigate reddening introduced by a beta angle flip. The upright
    orientation is such that north is up. The nightside
    data shown is the NO nightglow.
    ...
    # Save the graphic
    #save_location = iuvs_images_location / 'apoapse' / 'muv' / 'globe-heq-upright-nightside-reversed'
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-nightside-no_nightglow-reversed.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
...
def plot_apoapse_muv_daynight_globe_upright_aurora(orbit: int) -> None:
    """Plot an upright globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. The upright
    orientation is such that north is up. The nightside
    data shown is the aurora.
    ...
    # Load in datasets for the nightside image
    species_brightness = f['apoapse/muv/nightside/species/co_cameron_bands'][:] + f['apoapse/muv/nightside/species/co2+_uvd'][:]
    nightside_spatial_bin_vector = f['apoapse/muv/nightside/spatial_bin_geometry/bin_vector'][:]  # (integration, spatial bin, 5, 3)
    spacecraft_position = f['apoapse/spacecraft_geometry/iau_mars_frame/spacecraft_position'][~dayside_science_integrations]
    app_flip = f['apoapse/instrument_geometry/app_flip'][0]
    ...
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-nightside-aurora.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
    
def plot_apoapse_muv_daynight_globe_upright_aurora_reversed(orbit: int) -> None:
    """Plot an upright globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. The dayside swaths are plotted
    in reverse order to mitigate reddening from a beta angle flip. The upright
    orientation is such that north is up. The nightside
    data shown is the aurora.
    ...
    save_location = Path('/Volumes/iuvs_science/images/gil/globes-heq/')
    orbit_code = pu.make_orbit_code(orbit)
    filename = f'{orbit_code}_globe-heq-upright-nightside-aurora-reversed.png'
    save = save_location / pu.make_orbit_block(orbit) / filename
    save.parent.mkdir(parents=True, exist_ok=True)
    plt.savefig(save, dpi=150)
    plt.close(fig)
...
def plot_apoapse_muv_daynight_globe_rotated_no_nightglow(orbit: int) -> None:
    """Plot a rotated globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. The rotation makes
    the globe appear in the same orientation that IUVS saw Mars. The nightside
    data shown is the NO nightglow data.
    ...
    # Load in datasets for the nightside image
    species_brightness = f['apoapse/muv/nightside/species/no_nightglow'][:]
    nightside_spatial_bin_vector = f['apoapse/muv/nightside/spatial_bin_geometry/bin_vector'][:]
    spacecraft_position = f['apoapse/spacecraft_geometry/iau_mars_frame/spacecraft_position'][~dayside_science_integrations]
    ...
    # Make the dimensions of pixel grid and width of a pixel in kilometers
    xsize_night = int(8000 / pixel_size)
    ysize_night = int(8000 / pixel_size)
...
def plot_apoapse_muv_daynight_globe_rotated_aurora_colorbar(orbit: int) -> None:
    """Plot a rotated globe of the apoapse MUV data with HEQ coloring,
    projected from the point of apoapsis for a given orbit. The rotation makes
    the globe appear in the same orientation that IUVS saw Mars. The nightside
    data shown is the aurora.
    ...
    # Plot the nightside data
    x_night, y_night = np.meshgrid(np.linspace(-4000, 4000, xsize_night), np.linspace(-4000, 4000, ysize_night))
    n = nightside_ax.pcolormesh(x_night, y_night, night_grid, cmap=pu.graphics.co2p_colormap,
                            norm=matplotlib.colors.SymLogNorm(linthresh=1, vmin=0, vmax=4), rasterized=True)
    ...
    # Create nightside colorbar axis 
    nightside_cb_ax = plt.axes((0.095, 0.095, 0.87, 0.87))
    matplotlib.rcParams.update({'text.color' : "white", 
                                'axes.labelcolor' : "white"})
    
    nightside_cb_ax.set_xticks([])
    nightside_cb_ax.set_xlim(-4000, 4000)
    nightside_cb_ax.set_yticks([])
    nightside_cb_ax.set_ylim(-4000, 4000)
    nightside_cb_ax.set_frame_on('False')
    nightside_cb_ax.patch.set_visible(False)
    nightside_cb_ax.spines[['right', 'top', 'left', 'bottom']].set_visible(False)
    
    divCO1 = make_axes_locatable(nightside_cb_ax)
    cax_co1 = divCO1.append_axes("right", size="3%", pad=0.000005)
    cb = plt.colorbar(n, cax=cax_co1)
    cb.set_label('Intensity (kR)', labelpad=-10)
...
if __name__ == '__main__':
    for orb in range(21222, 21550):
        print(orb)
        try:
            plot_apoapse_muv_daynight_globe_upright_aurora(orb)
            plot_apoapse_muv_daynight_globe_upright_no_nightglow(orb)
            plot_apoapse_muv_daynight_globe_upright_aurora_reversed(orb)
            plot_apoapse_muv_daynight_globe_upright_no_nightglow_reversed(orb)
            plot_apoapse_muv_dayside_globe_upright(orb)
            plot_apoapse_muv_dayside_globe_upright_reversed(orb)
        except IndexError:
            print(f"{orb} failed")
